...
import { RefreshIcon, DownloadIcon, MaleIcon, FemaleIcon, GenerateIcon } from './components/IconComponents';
...
const App: React.FC = () => {
  ...
  const GenderButton: React.FC<{ value: Gender; label: string; icon: React.ReactNode }> = ({ value, label, icon }) => {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 flex flex-col">
        ...
        <div className="flex flex-col items-center justify-center space-y-6 px-4">
            <div className="w-full space-y-3">
                <div className="grid grid-cols-2 gap-4">
                    <GenderButton value={Gender.MALE} label="남자아이" icon={<MaleIcon className="w-6 h-6" />} />
                    <GenderButton value={Gender.FEMALE} label="여자아이" icon={<FemaleIcon className="w-6 h-6" />} />
                </div>
            </div>
      ...
      <footer className="text-center mt-8 text-pink-500">
        <p>사용 방법: 1. 아이의 정면 사진을 업로드하세요. 2. 성별을 선택하세요. 3. '생성' 버튼을 누르고 잠시 기다려주세요.</p>
        <p className="text-xs mt-2 text-pink-400">본 결과는 AI에 의해 생성된 가상의 이미지이며 실제와 다를 수 있습니다.</p>
      </footer>
    </div>
  );
};
